...
*** General                                       :ignore:
    #+begin_export latex
     \begin{quote}
     ``Through perseverance and commitment, I've helped a startup achieve a successful exit, and I'm ready to take on the next one! My diverse technical background in software development has helped me solve countless challenges while my calm, attentive personality endears me to clients and fellow engineers alike.''
     \end{quote}
    #+end_export
...
*** Senior Software  Engineer                                        :ignore:
    {{{cvevent(Senior Software Engineer, Peruse Technology Inc (SOLD), Sep 2022 - Current, Remote)}}}

    - Saw through the development of a product and platform which grew revenue while on-boarding new clients which allowed us to achieve a successful exit.
    - Implemented many critical features of a high-volume logistics document processing platform including automatic broker verification, automatic and manual document assignment, custom rules engine, as well as the overall load processing pipeline used by both UI and API customers as well as technical challenges such as  a database migration from {{{cvtag(Datomic)}}} to {{{cvtag(Postgres)}}}.
    - Full stack development spanning from backend {{{cvtag(REST)}}} and {{{cvtag(GraphQL)}}} APIs written in {{{cvtag(Clojure)}}} and {{{cvtag(Python)}}}, frontend {{{cvtag(Typescript)}}} UI components, and cloud-based infrastructure maintained using {{{cvtag(terraform)}}} and deployed to {{{cvtag(AWS)}}} while also maintaining and deploying LLM-based machine learning models trained using {{{cvtag(PyTorch)}}}.
...
*** PhD Researcher :ignore:
    ...
    - Managed interconnected computational projects written in {{{cvtag(Python)}}}, {{{cvtag(Java)}}}, and {{{cvtag(Clojure)}}} across 4+ teams through consistent communication and leadership and produced 6 publications, 4 dataset releases, and 5 package releases.
...
*** IDEAL Invited Speaker                                            :ignore:

    {{{cvevent(IDEAL Invited Speaker, The Institute for Data\, Econometrics\, Algorithms\, and Learning, June 2024, Chicago\, IL)}}}
    
    - Title: AI in the Transportation Sector: Challenges and Successes
